 ...
 ************************************************************************
  ...
  THIS IS AN EXAMPLE OF A SAS PROGRAM THAT LINKS NHIS DATA FOR 2004 FORWARD, DOWNLOADED
  FROM THE NCHS WEBSITE, AND IHIS DATA IN SAS FILE FORMAT.

  NOTES:
    CERTAIN VARIABLES IN FOR LINKING, INCLUDING SRVY_YR HHX FMX FPX,         
  	IN NHIS DATA ARE NEEDED AND NOT TO BE CHANGED BEFORE THE MERGE 
...
PROC SORT DATA=&nhisdsname (keep=hhx fmx fpx)  OUT=A1 NODUPKEY;
BY hhx fmx fpx;
RUN;
...
nhispid=compress("00"||put(srvy_yr,4.)||hhx||fmx||fpx);
RUN;
...
PROC FREQ DATA=&dsmerge;
  BY year NOTSORTED;
  TABLES _merge / OUT=__freqs outcum;
Run;
DATA _NULL_;
  SET __freqs;
  BY year;
  ...
  FILE log;
  RETAIN yearcum yearpct 0;
  FORMAT _merge;

  IF first.year THEN
    PUT // ' CHECK THE RESULTS OF THE MERGE DATA'/
	       '                                             '/
           'year=' year /
           '    _merge |    Freq.    Percent         Cum.'/
           '---------------------------------------------';
    PUT @10 _merge '| ' count comma7. +6 percent 6.2 +7 cum_pct 6.2;

  IF last.year THEN PUT
         '---------------------------------------------' /
         @6 'Total | ' cum_freq comma7. +6 cum_pct 6.2 +7 cum_pct 6.2;
RUN;
