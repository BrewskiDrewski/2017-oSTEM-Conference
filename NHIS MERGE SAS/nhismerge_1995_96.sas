 ...
 ************************************************************************
  ...
  THIS IS AN EXAMPLE OF A SAS PROGRAM THAT LINKS 1995 OR 1996 NHIS DATA DOWNLOADED
  FROM THE NCHS WEBSITE AND IHIS DATA IN SAS FILE FORMAT.

  NOTES:
    CERTAIN VARIABLES IN FOR LINKING, INCLUDING YEAR HHID PNUM,         
...
PROC SORT DATA=&nhisdsname (keep=hhid pnumm)  OUT=A1 NODUPKEY;
BY hhid pnumm;
RUN;
...
nhispid=compress(put(year,4.)||hhid||pnum);
RUN;
