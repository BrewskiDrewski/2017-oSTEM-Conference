 ...
 ************************************************************************
  ...
  THIS IS AN EXAMPLE OF A SAS PROGRAM THAT LINKS NHIS DATA FOR SURVEY YEARS 1997-2003, DOWNLOADED FROM THE NCHS WEBSITE AND IHIS DATA IN SAS FILE FORMAT.

  NOTES:
    CERTAIN VARIABLES IN FOR LINKING, INCLUDING SRVY_YR HHX FMX PX,         
...
%let ihisdsname=ihis.<insert IHIS data file name here>;
...
PROC SORT DATA=&nhisdsname (keep=hhx fmx px)  OUT=A1 NODUPKEY;
BY hhx fmx px;
RUN;
...
nhispid=compress("00"||put(srvy_yr,4.)||hhx||fmx||px);
RUN;
