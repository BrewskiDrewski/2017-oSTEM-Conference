 ...
 ************************************************************************
 SEPTEMBER 16, 2013
  ...
  THIS IS AN EXAMPLE OF A SAS PROGRAM THAT LINKS NHIS DATA FOR 1982 TO 1991, 1993 OR 1994, DOWNLOADED
  FROM THE NCHS WEBSITE, AND IHIS DATA IN SAS FILE FORMAT.

  NOTES:
    CERTAIN VARIABLES IN FOR LINKING, INCLUDING YEAR QUARTER PSUNUMR WEEKCEN          
...
PROC SORT DATA=&nhisdsname (keep=quarter psunumr weekcen segnum hhnum pnum)  OUT=A1 NODUPKEY;
BY quarter psunumr weekcen segnum hhnum pnum;
RUN;
...
nhispid=compress(put(year,4.)||put(quarter,1.)||psunumr||put(weekcen,z2.)||segnum||hhnum||pnum);
RUN;
